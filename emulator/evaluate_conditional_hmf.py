...
def main() -> None:
    ...
    fig, (ax, ax_cdf) = plt.subplots(2, 1, figsize=(7.5, 8.2), sharex=True, constrained_layout=True)
    ...
    obs_sum = float(np.sum(obs_tot))
    pred_sum = float(np.sum(pred_tot))
    obs_cdf = np.cumsum(obs_tot) / (obs_sum if obs_sum > 0 else 1.0)
    pred_cdf = np.cumsum(pred_tot) / (pred_sum if pred_sum > 0 else 1.0)
    ax_cdf.plot(log10M_centers, obs_cdf, lw=2, label="Observed CDF")
    ax_cdf.plot(log10M_centers, pred_cdf, lw=2, label="Predicted CDF")
    ax_cdf.set_ylim(0.0, 1.0)
    ax_cdf.set_xlabel(r"$\log_{10}(M / 10^{10}\,M_\odot)$")
    ax_cdf.set_ylabel("CDF of counts")
